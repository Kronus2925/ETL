...
def main():
    ...
    for i in category:
        ...
        get_sql_values(mainlist,db,name)
...
def get_sql_table(category, mainlist):
    all_keys = set(key for d in mainlist for key in d.keys())
    columns = " ,".join(all_keys)
    db = Database('Hurtownia_Zabawek.db')
    db.execute(f'''CREATE TABLE IF NOT EXISTS {category} ({columns})''')
    return db


def get_sql_values(mainlist, db, category):
    df = pd.DataFrame(mainlist)
    df.to_sql(category, db.connect(), if_exists='replace',index=False)
